...
INFO_KEYWORDS = (
    ...
    "ep_r_drop", "ep_tag_visible_mean", "ep_tag_stale_mean",
)
...
class SpinEvalCallback(BaseCallback):
    ...
    def __init__(self, eval_env, every_steps: int, save_dir: str,
        ...
        # Selection key is (success_rate, revolutions), compared lexically.
        # success_rate dominates -- "did it complete a turn without dropping
        # the cube" is the thing that has to be true before "how many
        # revolutions" means anything. Revolutions only break ties among
        # checkpoints that are equally successful. Using mean revolutions
        # alone (the old `self.best = rev`) rewards a policy that spins fast
        # and gets dropped, because r_rot pays every step up to the drop
        # while drop_penalty is a single one-time cost -- see the run-2
    ...
    def _on_step(self) -> bool:
        ...
        print(f"[EVAL {self.num_timesteps}] revolutions={rev:.3f} "
              ...
              f"(best success={self.best[0]:.3f} rev={self.best[1]:.3f})")
        ...
        if score > self.best:
            ...
            print(f"[EVAL] new best (success={succ:.3f} rev={rev:.3f}) saved")
        return True
...
class DRCurriculumCallback(BaseCallback):
    """Ramps `env.dr_scale` from `start` to 1.0 as the policy starts actually
    solving the task, instead of training against the full-strength physics
    + pose-sensor/servo noise from step 0.

    The pose sensor (GTPoseSensor by default) is NEVER bypassed at any scale.
    This only widens how hard a draw from it is, and only once the rolling mean of `success` (the
    Monitor's `success` key, aggregated by SB3 into `ep_info_buffer`) clears
    `threshold` over the last `window` episodes. That keeps the ramp keyed to
    evidence the policy can already do SOMETHING at the current scale, rather
    than a fixed step schedule that might race ahead of it.
    """

    def __init__(self, start: float = 0.25, step: float = 0.1, threshold: float = 0.05,
                 window: int = 40, check_every_steps: int = 20_000):
        super().__init__()
        self.scale = start
        self.step = step
    ...
    def _apply(self):
        self.training_env.env_method("set_dr_scale", self.scale)

    def _on_training_start(self) -> None:
        self._apply()

    def _on_step(self) -> bool:
        freq = max(self.check_every_steps // self.training_env.num_envs, 1)
        if self.n_calls % freq != 0 or self.scale >= 1.0:
            return True
        buf = self.model.ep_info_buffer
        if buf is None or len(buf) < self.window:
            return True
        recent = list(buf)[-self.window:]
        succ = float(np.mean([ep.get("success", 0.0) for ep in recent]))
        self.logger.record("curriculum/dr_scale", self.scale)
        self.logger.record("curriculum/success_recent", succ)
        if succ >= self.threshold:
            self.scale = min(1.0, self.scale + self.step)
            self._apply()
            print(f"[Curriculum] success={succ:.3f} >= {self.threshold} over last "
                  f"{self.window} eps -> dr_scale={self.scale:.2f}")
        return True
...
def main(args) -> None:
    ...
    # Cube-mesh size is compile-time in MuJoCo (a mesh geom's `size` field is
    # ignored) so it cannot be jittered inside one running env the way mass/
    # friction/damping are. The practical DR for it is offline: pre-compile a
    # few scene XMLs at different cube scales and round-robin them across the
    # PARALLEL envs, so the population the policy trains against still spans
    # a size range -- just at env granularity instead of per-episode.
    if args.scene_variants:
        variants = [os.path.abspath(p) for p in args.scene_variants.split(",")]
        for p in variants:
            if not os.path.exists(p):
                raise FileNotFoundError(f"scene variant not found: {p}")
        print(f"[Setup] cube-size DR across {len(variants)} scene variants "
              f"(round-robin over {n_envs} envs)")

        def _make_env(rank):
            kw = dict(env_kw, model_path=variants[rank % len(variants)])
            def _init():
                env = AmazingHandSpinEnv(**kw)
                return Monitor(env, os.path.join(log_dir, f"train_{rank}"),
                              info_keywords=INFO_KEYWORDS)
            return _init

        train_env = SubprocVecEnv([_make_env(i) for i in range(n_envs)])
    ...
    if args.dr_curriculum:
        callbacks.append(DRCurriculumCallback(
            start=args.dr_start, step=args.dr_step,
            threshold=args.dr_threshold, window=args.dr_window))
...
if __name__ == "__main__":
    ...
    p.add_argument("--dr-curriculum", action="store_true",
                   help="ramp env.dr_scale from --dr-start to 1.0 as `success` "
                        "clears --dr-threshold; the pose sensor stays active "
                        "at every scale, only how hard the draw is changes")
    p.add_argument("--dr-start", type=float, default=0.25)
    p.add_argument("--dr-step", type=float, default=0.1)
    p.add_argument("--dr-threshold", type=float, default=0.05)
    p.add_argument("--dr-window", type=int, default=40,
                   help="episodes of rolling `success` the curriculum checks")
    ...
    p.add_argument("--scene-variants", type=str, default=None,
                   help="comma-separated scene XMLs at different cube/tag "
                        "scales, round-robined across --n-envs for offline "
                        "size domain randomisation (mesh size is compile-"
                        "time in MuJoCo, so this replaces --scene)")
